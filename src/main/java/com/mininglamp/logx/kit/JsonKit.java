...
public class JsonKit {
		...
		if(json.startsWith("{")&&json.endsWith("}")){
			...
			while(rankIdx < jsonLen-1){
				...
				if(rankChar == ':'&&closeFlag == 0){
					_key = json.substring(preIdx,rankIdx).trim();
					preIdx = rankIdx + 1;
					continue;
				}
				//value
				if(rankChar == ','&&closeFlag == 0){
					_value = json.substring(preIdx,rankIdx).trim();
					map.put(handleKey(_key), parseValue(handleValue(_value)));
					preIdx = rankIdx+1;
				}
			}
			
			_value = json.substring(preIdx).trim();
			map.put(handleKey(_key), parseValue(_value));
	...
	private static String handleKey(String _key){
		if(_key == null||"".equals(_key.trim())) return null;
		if(_key.startsWith("\"")&&_key.endsWith("\"")&&_key.length()>2)
	 ...
	 * @return
	 */
	private static Object handleValue(String _value){
		if(_value == null||"".equals(_value.trim())) return null;
		if(_value.startsWith("\"")&&_value.endsWith("\"")&&_value.length()>2)
			_value = _value.substring(1,_value.length()-1);
		try{
			Integer intVal = Integer.parseInt(_value);
			return intVal;
		}catch(NumberFormatException e){
		}
		return _value;
